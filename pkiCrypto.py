...
def generate_private_key(openssl, key_file, algorithm):
    if algorithm in classical_algorithms:
        if algorithm == 'rsa2048':
            ...
            cmd = f"{openssl} genpkey -algorithm RSA -out {key_file} -pkeyopt rsa_keygen_bits:3072"
...
def sign_certificate(openssl, csr_file, crt_file, ca_key, ca_passfile, ca_cert, ca_conf, ext):
    try:
        ...
        if os.path.isfile(crt_file):
            print(f"Successfully created end entity certificate: {crt_file}")
            return
        else:
    ...
    try:
        with open(chain_file, "wb") as chain:
            ...
            with open(ca_chain, "rb") as ca:
                ...
                chain.write(ca.read())
                ca.close()
        print(f"Certificate chain successfully created: {chain_file}")
        chain.close()
    except Exception as e:
        print(f"Error creating certificate chain: {e}")
