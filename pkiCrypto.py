...
def generate_private_key(openssl, pki, key_file, algorithm):
    if pki != "certs":
        openssl = f'OPENSSL_CONF={shlex.quote(OQS_CONF)} OPENSSL_MODULES={shlex.quote(OPENSSL_MODULES)} {openssl}'
...
def generate_csr(openssl, pki, private_key, csr_filename, subject, conf, commonName, subjectAltName, cn_type):
    env = os.environ
    logging.info(f"Config file: {conf}")
    if pki != "certs":
        openssl = f'OPENSSL_CONF={shlex.quote(OQS_CONF)} OPENSSL_MODULES={shlex.quote(OPENSSL_MODULES)} {openssl}'
    if subjectAltName != "":  
        # server or client
...
def get_ca_certificate_details(openssl, ca_cert_path, chain):

    if chain != "certs":
        openssl_cmd = f'OPENSSL_CONF={shlex.quote(OQS_CONF)} OPENSSL_MODULES={shlex.quote(OPENSSL_MODULES)} {openssl}'
    else:
        openssl_cmd = openssl
    cert_command = f"{openssl_cmd} x509 -in {ca_cert_path} -noout -text -certopt no_sigdump"
    ca_cert_data = ""
    ca_cert_data = subprocess.check_output(cert_command, shell=True, text=True).strip()
    return ca_cert_data

def get_crl_details(openssl, crl_path, chain):
    if chain != "certs":
        openssl = f'OPENSSL_CONF={shlex.quote(OQS_CONF)} OPENSSL_MODULES={shlex.quote(OPENSSL_MODULES)} {openssl}'
